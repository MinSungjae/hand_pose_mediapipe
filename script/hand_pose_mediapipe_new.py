...
class HandLandmarkerNode:
                ...
                for idx, landmarks in enumerate(self.latest_result.hand_landmarks):
                    ...
                    for finger_name, (mcp_i, pip_i, dip_i, tip_i) in fingers.items():
                        ...
                        if finger_name == "Thumb":
                            ...
                            # Flexion: Thumb base (1) - MCP (2) - IP (3)
                            mcp_flex = self.calculate_angle(pts[1], pts[2], pts[3], in_degrees=False)

                            # Abduction with stability + optional filtering
                            mcp_abd = self.compute_thumb_abduction(
                                mcp, pip, wrist, index_mcp, pinky_mcp, prev_abd=self.prev_thumb_abd
                            )
                            self.prev_thumb_abd = mcp_abd
                        else:
                            smcp_flex = self.calculate_angle(wrist, mcp, pip, in_degrees=False)
                            smcp_abd = self.compute_abduction_from_hand_center(mcp, pip, x_axis, palm_normal)
    ...
    def compute_thumb_abduction(self, mcp, pip, wrist, index_mcp, pinky_mcp, prev_abd=None, alpha=0.8):
        # 손바닥 좌우 축
        ...
        # 평면에 투영
        proj = thumb_vec - np.dot(thumb_vec, palm_normal) * palm_normal
        ...
        if norm_proj < 1e-3:
            ...
            proj /= norm_proj
            angle = np.arccos(np.clip(np.dot(proj, palm_x), -1.0, 1.0))
            sign = np.sign(np.dot(np.cross(palm_x, proj), palm_normal))
            abd_angle = angle * sign

        # 선택적으로 low-pass filter
        if prev_abd is not None:
            abd_angle = alpha * prev_abd + (1 - alpha) * abd_angle
